...
def risk_schema_designer_node(state: AgentState, config: RunnableConfig) -> Dict:
    """
    ...
    # Perform initial searches to gather company information
    search_queries = [
        f"{company_name} latest annual report 10-K risk factors",
        f"{company_name} recent earnings call investor presentation",
        f"{company_name} credit rating report"
    ]
    ...
    for query in search_queries:
        ...
        try:
            response = tavily_client.search(query=query, max_results=3, include_raw_content=False)
    ...
    rate_limit_delay()
    result = schema_llm.invoke({"company_name": company_name, "search_context": search_context})
...
def risk_assessor_node(state: AgentState, config: RunnableConfig) -> Dict:
    """
    ...
    # Perform searches for risk assessment
    search_queries = [
        f"{company_name} financial statements latest",
...
def risk_validator_node(state: AgentState, config: RunnableConfig) -> Dict:
    """
...
def tavily_search_node(state: ResearchState, config: RunnableConfig):
    ...
    tavily_client = TavilyClient("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")
...
def result_accumulator_node(state: ResearchState, config: RunnableConfig):
    ...
    # Truncate search results to avoid token limit errors
